...
const IMPORTANCE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  high: { label: "重要", color: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300" },
...
export default function RankingInsightTab({ managedTags, showToast }: Props) {
  ...
  const handleRun = async () => {
    try {
      ...
      showToast("ランキングインサイト分析が完了しました", "success");
    } catch (e: any) {
      showToast(e.message ?? "分析に失敗しました", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-800 p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-amber-900 dark:text-amber-300 mb-1">ランキング × 媒体 × ペルソナ分析</h3>
        <p className="text-xs text-amber-600 dark:text-amber-400 mb-3">
          ランキング上位/下位の動画を媒体ごとに比較し、ヒット要因とペルソナプロファイルを特定します。
        </p>
        <button onClick={handleRun} disabled={loading}
          ...
          {loading ? "分析中..." : "ランキングインサイト分析を実行"}
        </button>
        {loading && (
          <div className="mt-3 flex items-center gap-2">
            <div className="h-4 w-4 animate-spin rounded-full border-2 border-amber-500 border-r-transparent" />
            <span className="text-sm text-amber-600 dark:text-amber-400 animate-pulse">Gemini AIが分析中...</span>
          </div>
        )}
      </div>
      ...
      {result && !loading && (
        <div className="space-y-5">
                ...
                {result.platform_ranking_matrix.map((pm, i) => (
                  <div key={i} className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 pt-2 border-t border-gray-100 dark:border-gray-600">
                      <span className="font-medium">成功の法則:</span> {pm.platform_success_formula}
                    </p>
                  </div>
                ))}
              </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700">
                    ...
                    <th className="text-left py-2 px-3 font-medium text-green-600 dark:text-green-400">上位の特徴</th>
                    <th className="text-left py-2 px-3 font-medium text-red-600 dark:text-red-400">下位のギャップ</th>
                    <th className="text-left py-2 px-3 font-medium text-gray-500 dark:text-gray-400">有効媒体</th>
                  </tr>
                </thead>
                <tbody>
          ...
          {result.persona_profiles.length > 0 && (
            <div className="rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-5 shadow-sm">
              <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">ペルソナプロファイル（媒体別）</h4>
                    ...
                    <div className="space-y-1.5 text-xs">
                      ...
                      {pp.primary_persona.purchase_triggers.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          <span className="text-gray-500 dark:text-gray-400">購入トリガー:</span>
                      ...
                      {pp.primary_persona.content_preferences.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          <span className="text-gray-500 dark:text-gray-400">好みコンテンツ:</span>
          ...
          {result.cross_platform_persona_insights.length > 0 && (
            <div className="rounded-xl bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800 p-5">
              <h4 className="text-sm font-semibold text-teal-800 dark:text-teal-300 mb-3">クロスプラットフォームインサイト</h4>
                ...
                {result.content_strategy_by_platform.map((cs, i) => (
                  <div key={i} className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div>
                        <span className="font-medium text-gray-500 dark:text-gray-400">ペルソナメッセージング: </span>
                    ...
                    {cs.sample_script_outline && (
                      <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-600">
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">台本概要:</p>
