...
function stagePercent(stage: VideoTranscriptionStage): number {
  if (stage === "queued" || stage === "error") return 0;
...
function keyStateLabel(state: string): { label: string; className: string } {
  switch (state) {
    case "available":
      return { label: "待機", className: "bg-gray-100 text-gray-600" };
    case "working":
      return { label: "処理中", className: "bg-blue-100 text-blue-700 animate-pulse" };
    case "rate_limited":
      return { label: "制限中", className: "bg-yellow-100 text-yellow-700" };
    case "error":
      return { label: "エラー", className: "bg-red-100 text-red-700" };
    default:
      return { label: state, className: "bg-gray-100 text-gray-600" };
  }
}
...
function ElapsedTime({ startedAt, completedAt }: { startedAt?: number; completedAt?: number }) {
  const now = useTicker(Boolean(startedAt) && !completedAt);
  if (!startedAt) return null;
  const elapsed = Math.round(((completedAt ?? now ?? startedAt) - startedAt) / 1000);
  return <span className="text-[10px] text-gray-400 tabular-nums">{formatSeconds(elapsed)}</span>;
}
  ...
  return (
    <div className="flex items-center gap-0.5">
      {STAGES.map((step, index) => {
        const active = stage !== "queued" && stage !== "error" && index <= current;
        const isCurrent = stage !== "queued" && stage !== "error" && index === current && stage !== "completed";
...
export default function BatchProgressPanel({ progress, onCancel }: BatchProgressPanelProps) {
  ...
  const elapsed = Math.round(((now ?? progress.startedAt) - progress.startedAt) / 1000);
  const activeCount = videos.filter((video) => video.stage !== "queued" && video.stage !== "completed" && video.stage !== "error").length;
    ...
    <div className="rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden">
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-blue-200 px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-blue-800">
            {isCancelled ? "キャンセル中..." : isRunning ? "一括書き起こし実行中" : "一括書き起こし完了"}
          </span>
        </div>
      ...
      <div className="px-5 py-3 border-b border-gray-100">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-xs text-gray-500">全体進捗 {overallPercent}%</span>
          <div className="flex items-center gap-3 text-[11px] text-gray-500">
            ...
            {estimatedSecondsRemaining != null && (
              <span className="tabular-nums font-medium text-blue-600">残り {formatSeconds(estimatedSecondsRemaining)}</span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mt-2 flex-wrap">
          ...
          <span className="inline-flex items-center gap-1 text-[11px] text-gray-500">
            ...
            待機 {videos.filter((video) => video.stage === "queued").length}件
          </span>
      ...
      {keyStatuses.length > 1 && (
        <div className="px-5 py-2 border-b border-gray-100 flex items-center gap-2 flex-wrap">
          ...
          {keyStatuses.map((status) => {
            const { label, className } = keyStateLabel(status.state);
            return (
              <span
                key={status.index}
                className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${className}`}
                title={status.rateLimitedUntil ? `復帰: ${new Date(status.rateLimitedUntil).toLocaleTimeString()}` : undefined}
              >
                #{status.index + 1} {label}
                {status.completedCount > 0 && <span className="opacity-60">({status.completedCount}件)</span>}
              </span>
            );
          })}
        </div>
      ...
      <div className="px-5 py-1.5 border-b border-gray-100 flex items-center gap-4">
        ...
        {STAGES.map((stage, index) => (
          <span key={stage} className="text-[10px] text-gray-400 flex items-center gap-1">
      ...
      <div className="max-h-[420px] overflow-y-auto">
        <div className="divide-y divide-gray-100">
          {videos.map((video) => (
            <VideoRow key={video.videoId} vp={video} />
          ))}
        </div>
      </div>
    </div>
