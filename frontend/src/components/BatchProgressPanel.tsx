...
function stageColor(stage: VideoTranscriptionStage): string {
  switch (stage) {
    case "queued": return "text-gray-400 dark:text-gray-500";
    case "downloading": return "text-blue-500 dark:text-blue-400";
    case "preparing": return "text-blue-500 dark:text-blue-400";
    case "transcribing": return "text-yellow-600 dark:text-yellow-400 animate-pulse";
...
function keyStateLabel(state: string): { label: string; className: string } {
  switch (state) {
    case "available": return { label: "待機", className: "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300" };
    case "working": return { label: "処理中", className: "bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 animate-pulse" };
    case "rate_limited": return { label: "制限中", className: "bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300" };
    case "error": return { label: "エラー", className: "bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300" };
    default: return { label: state, className: "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300" };
  }
}
...
export default function BatchProgressPanel({ progress, onCancel }: BatchProgressPanelProps) {
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 border-b border-blue-200 dark:border-blue-800 px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {isRunning && !isCancelled && (
            <div className="h-4 w-4 animate-spin rounded-full border-2 border-blue-600 dark:border-blue-400 border-r-transparent" />
          )}
          {!isRunning && (
        ...
        <div className="flex items-center gap-3">
          ...
          {isRunning && !isCancelled && (
            <button
              onClick={onCancel}
              className="rounded-lg border border-red-300 dark:border-red-700 bg-white dark:bg-gray-800 px-3 py-1 text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
            >
              キャンセル
            </button>
          )}
      ...
      <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-700">
        ...
        {errorVideos > 0 && (
          <p className="mt-1 text-xs text-red-500 dark:text-red-400">{errorVideos}件のエラーが発生</p>
        )}
      </div>

      {/* API Key statuses */}
      {keyStatuses.length > 1 && (
        <div className="px-5 py-2 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2 flex-wrap">
          <span className="text-xs text-gray-400 dark:text-gray-500 mr-1">APIキー:</span>
          {keyStatuses.map((ks) => {
      ...
      <div className="max-h-72 overflow-y-auto">
        <div className="divide-y divide-gray-50 dark:divide-gray-700/50">
          {videos.map((vp) => (
            <div
              key={vp.videoId}
              className="flex items-center justify-between px-5 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
            >
              <div className="flex items-center gap-3 min-w-0 flex-1">
                {vp.stage === "completed" ? (
                  ...
                  <div className="h-5 w-5 shrink-0 animate-spin rounded-full border-2 border-blue-500 dark:border-blue-400 border-r-transparent" />
                )}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {vp.error && (
                    <span className="text-xs text-red-400 dark:text-red-500 truncate block">{vp.error}</span>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0 ml-3">
                {vp.apiKeyIndex != null && vp.stage !== "completed" && vp.stage !== "error" && vp.stage !== "queued" && (
                  <span className="text-xs text-gray-400 dark:text-gray-500">Key#{vp.apiKeyIndex + 1}</span>
                )}
